...
const App: React.FC = () => {
  ...
  // Sincronizar ref con estado para el listener
  useEffect(() => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.altKey && e.key === '1') {
        if (adminRef) {
          // Si ya es admin, lo desactivamos directamente
          setIsAdminMode(false);
          alert('Modo Administrador DESACTIVADO');
        } else {
          // Si no es admin, pedimos clave
  ...
  }, [adminRef]); // Dependencia adminRef para tener el valor actualizado
