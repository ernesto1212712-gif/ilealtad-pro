...
import { LayoutDashboard, Database, Search, Upload, LogOut, ShieldCheck, Users, Sun, Moon, Smartphone, GraduationCap, Globe, FilePlus, Gavel, Megaphone, Cloud, Wifi } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ onLogout, isAdminMode, theme, toggleTheme }) => {
  ...
  useEffect(() => {
    ...
    // Cargar anuncios cada vez que renderiza
    const loadSidebarAds = async () => {
      const allAds = await getAds();
      setAds(allAds.filter(a => a.type === 'sidebar'));
    };
    loadSidebarAds();
  }, [location]); // Recargar si cambia la ruta (por si se agregaron anuncios)
  ...
  const navItems = [
    ...
    { icon: FilePlus, label: 'Reportar Caso', path: '/submit-case' }, // Nueva opción pública
    { icon: Database, label: 'Base de Datos', path: '/database' },
  ];

  // Hidden items
  const adminItems = [
    { icon: Gavel, label: 'Moderación', path: '/moderation' }, // Nueva opción admin
    { icon: Megaphone, label: 'Gestor de Anuncios', path: '/manage-ads' }, // Nueva opción admin
    { icon: Upload, label: 'Importar Datos', path: '/upload' },
    { icon: Users, label: 'Registro de Accesos', path: '/admin-logs' },
  ];

  return (
    <div className="h-screen w-64 bg-slate-900 dark:bg-black text-white flex flex-col fixed left-0 top-0 shadow-xl z-20 border-r border-slate-800 transition-colors">
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
        ...
        <div className="pt-6 mt-2 border-t border-slate-800 space-y-4">
          ...
          {/* Default Ads if no dynamic sidebar ads exist, or mix them */}
          ...
          {/* Dynamic Sidebar Ads Added by Admin */}
          {ads.map(ad => (
      ...
      <div className="p-2">
         <button
          onClick={onLogout}
          className="flex items-center justify-center gap-2 w-full py-2 text-slate-500 hover:text-red-400 text-xs font-medium hover:bg-slate-800 rounded transition-all"
        >
          <LogOut size={14} />
