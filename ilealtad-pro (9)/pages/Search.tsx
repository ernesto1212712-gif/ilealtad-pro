...
import { Search as SearchIcon, MapPin, FileText, User, Activity, AlertTriangle, CheckCircle, Eye, X, Link as LinkIcon } from 'lucide-react';
                    ...
                    <div className="flex-1">
                      ...
                      {item.socials && (
                          <div className="mt-3 flex items-center gap-2 text-sm text-slate-500">
                              <LinkIcon size={14} />
                              <span className="truncate max-w-md">{item.socials}</span>
                          </div>
                      )}
                    </div>
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
               {selectedCase.socials && (
                   <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-lg border border-slate-100 dark:border-slate-700">
                       <h4 className="text-xs font-bold text-slate-500 uppercase mb-1">Redes Sociales / Enlaces</h4>
                       <p className="text-sm text-brand-600 dark:text-brand-400 break-words">{selectedCase.socials}</p>
                   </div>
               )}
               <div>
