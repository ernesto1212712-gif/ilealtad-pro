...
import { FilePlus, Send, CheckCircle, Link as LinkIcon } from 'lucide-react';
  ...
  if (submitted) {
    return (
      <div className="p-8 max-w-2xl mx-auto text-center mt-20 animate-in fade-in slide-in-from-bottom-4">
        ...
        <button 
          onClick={() => { setSubmitted(false); setFormData({name:'', dni:'', location:'', details:'', socials: ''}); }}
    ...
    <div className="p-8 max-w-3xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-500 dark:text-slate-400">
          Ingrese los datos del objetivo. La información será verificada por moderación.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
             <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Redes Sociales / Enlaces (Opcional)</label>
             <div className="relative">
                <LinkIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                <input
                  type="text"
                  className="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none dark:text-white"
